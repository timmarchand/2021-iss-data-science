...
This tutorial has been adapted from  material produced by the authors of the [`moderndive`](https://moderndive.com/index.html) package (Kim & Ismay, 2020).

By now you should be familiar with the concept of a data frame in R: a rectangular spreadsheet-like representation of data where the rows correspond to observations and the columns correspond to variables describing each observation.  

In this tutorial, we explore what it means to have "tidy" data. You will see that having data stored in "tidy" format is about more than just having your data "neatly organized." Instead, lets define the term "tidy" as it's used by data scientists who use R, outlining a set of rules by which data is saved.

Knowledge of this type of data formatting was not necessary for most of the  data visualization in and data wrangling tasks we have done so far. This is because all the data used were already in "tidy" format. But when you start using data "in the wild" (web-scraped for example), you will start encountering data that needs a lot of processing to put it into a *tidy* format.

In this tutorial, we'll now see that this format is essential to use the tools we have covered up until now. 
...
```{r message=FALSE}
pacman::p_load(tidyverse, nycflights13, fivethirtyeight)
# library(dplyr)
# library(ggplot2)
# library(readr)
# library(tidyr)
...
# library(knitr)
# library(readxl)
pacman::p_load(kableExtra, stringr, scales, knitr, readxl, here)

dem_score <-  read_excel(here::here("data","dem_score.xlsx"))
```
...
Second, an Excel `.xlsx` spreadsheet file. This format is based on Microsoft's proprietary Excel software. As opposed to bare-bones `.csv` files, `.xlsx` Excel files contain a lot of meta-data (data about data). Some examples of Excel spreadsheet meta-data include the use of bold and italic fonts, colored cells, different column widths, and formula macros.
...
Another way is to publish it as a .csv file on Google Sheets; instructions on how to do this are in Steps 1-6 [here](https://twitter.com/rudeboybert/status/1055821846539591680).
...
In this `dem_score` data frame, the minimum value of `-10` corresponds to a highly autocratic nation, whereas a value of `10` corresponds to a highly democratic nation. Note also that backticks surround the different variable names.  Variable names in R by default are not allowed to start with a number nor include spaces, but we can get around this fact by surrounding the column name with backticks. We'll revisit the `dem_score` data frame soon.

Note that the `read_csv()` function included in the `readr` package is different than the `read.csv()` function that comes installed with R. The `read_csv()` function is easier to use since it can more easily read data off the web and generally imports data at a much faster speed. Furthermore, the `read_csv()` function included in the `readr` saves data frames as `tibbles` by default. 


### Using RStudio's interface

Let's read in the exact same data, but this time from an Excel file saved on your computer. Furthermore, we'll do this using RStudio's graphical interface instead of running `read_csv()` in the console. If you haven't found it in your data folder after applying the `update_project()` function, you can download the Excel file `dem_score.xlsx` by going to <a href="https://moderndive.com/data/dem_score.xlsx" download>https://moderndive.com/data/dem_score.xlsx</a>, then
...
At this point, you should see a screen pop-up like in Figure 1. After clicking on the "Import" button on the bottom right of Figure 1, RStudio will save this spreadsheet's data in a data frame called `dem_score` and display its contents in the spreadsheet viewer. 

```{r read-excel, echo=FALSE, fig.cap="Figure 1: Importing an Excel file to R.", purl=FALSE}
knitr::include_graphics("../figures/read_excel.png")
```

Furthermore, note the "Code Preview" block in the bottom right of Figure 1. You can copy and paste this code to reload your data again later programmatically, instead of repeating this manual point-and-click process.

## "Tidy" data {#tidy-data-ex}

Let's learn about the concept of "tidy" data format with a motivating example from the `fivethirtyeight` package. The `fivethirtyeight` package [@R-fivethirtyeight] provides access to the datasets used in many articles published by the data journalism website, [FiveThirtyEight.com](https://fivethirtyeight.com/). For a complete list of all `r nrow(data(package = "fivethirtyeight")[[3]])` datasets included in the `fivethirtyeight` package, check out the package webpage by going to: <https://fivethirtyeight-r.netlify.app/articles/fivethirtyeight.html>.\index{R packages!fivethirtyeight}
...
Let's apply some of the data wrangling verbs we have learned on the `drinks` data frame, `filter()` then `select()` then `rename()`:
...
Let's now ask ourselves a question: "Using the `drinks_smaller` data frame, how would we create the side-by-side barplot in Figure 2?".

```{r drinks-smaller, fig.cap=" Figure 2: Comparing alcohol consumption in 4 countries.", fig.height=3.9, echo=FALSE, purl=FALSE}
...
Observe that `drinks_smaller` has three separate variables `beer`, `spirit`, and `wine`.  In order to use the `ggplot()` function to recreate the barplot in Figure 2 however, we need a *single variable* `type` with three possible values: `beer`, `spirit`, and `wine`.  We could then map this `type` variable to the `fill` aesthetic of our plot.  In other words, to recreate the barplot in Figure 2, our data frame would have to look like this:
...
What does it mean for your data to be "tidy"? While "tidy" has a clear English meaning of "organized," the word "tidy" in data science using R means that your data follows a standardized format. We will follow Hadley Wickham's definition of *"tidy" data* \index{tidy data} [@tidy] shown also in Figure 3:
...
```{r tidyfig, echo=FALSE, fig.cap="Figure 3", out.height="80%", out.width="80%", purl=FALSE}
...
Although the data are neatly organized in a rectangular spreadsheet-type format, they do not follow the definition of data in "tidy" format. While there are three variables corresponding to three unique pieces of information (date, stock name, and stock price), there are not three columns. In "tidy" data format, each variable should be its own column, as shown in Table 1. Notice that both tables present the same information, but in different formats. 
...
Now we have the requisite three columns Date, Stock Name, and Stock Price. On the other hand, consider the data in Table 1.
...
stocks %>%
  kable(
    digits = 2,
    caption = "Table 1: Example of tidy data" # ,
    #    booktabs = TRUE
  ) %>%
  kable_styling(
    font_size = ifelse(is_latex_output(), 10, 16),
...
In this case, even though the variable "Boeing Price" occurs just like in our non-"tidy" data in Table 1, the data *is* "tidy" since there are three variables corresponding to three unique pieces of information: Date, Boeing price, and the Weather that particular day.
...
With our `drinks_smaller_tidy` "tidy" formatted data frame, we can now produce the barplot you saw in Figure 2 using `geom_col()`. This is done in Figure 4).
...
Comparing alcohol consumption in `r n_countries` countries using geom_col().

```{r drinks-smaller-tidy-barplot, echo=FALSE, fig.cap="Figure 4", fig.height=2.5, purl=FALSE}
if (is_html_output()) {
  drinks_smaller_tidy_plot
} else {
  drinks_smaller_tidy_plot + scale_fill_grey()
...
Furthermore, we'll make use of functions from the `ggplot2` and `dplyr` packages to produce a *time-series plot* showing how the democracy scores have changed over the 40 years from 1952 to 1992 for Guatemala. For time-series plots we often use `geom_line()`. 

Let's use the `dem_score` data frame we imported before, but focus on only data corresponding to Guatemala.
...
We can now create the time-series plot in Figure 5 to visualize how democracy scores in Guatemala have changed from 1952 to 1992 using a `geom_line()`. Furthermore, we'll use the `labs()` function in the `ggplot2` package to add informative labels to all the `aes()`thetic attributes of our plot, in this case the `x` and `y` positions.

```{r guat-dem-tidy, fig.cap="Figure 5: Democracy scores in Guatemala 1952-1992.", fig.height=3}
...
```{r}
## Convert dem_score into a tidy data frame
...
```{r}
## Read in the life expectancy data and convert it to tidy format
