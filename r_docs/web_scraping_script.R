...
marketwatch_wbpg %>%
  html_nodes(xpath =".block:nth-child(16) a , .block:nth-child(15) a , .block:nth-child(14) a , .block:nth-child(13) a , .block:nth-child(12) a , .block:nth-child(11) a , .block:nth-child(10) a , .block:nth-child(9) a , .block:nth-child(8) a , .block:nth-child(5) a , .resultlist .block:nth-child(3) a , .block:nth-child(1) a") %>% #See HTML source code for data within this tag
  html_text()
...
# Add http to  urls that do not vae fully formed urls

urls2 <-  c()
for(i in urls){
         correct_url <- ifelse(grepl("http", i),i,paste0("http://www.marketwatch.com", i)) # add correct http
  urls2 <- append(urls2, correct_url)
}
...
# Convert all ET (US Eastern Time) datetime values to 
# your local time - e.g. Japan
datetime_convert <- ymd_hms(
  datetime_parse, tz = "US/Eastern"
)
datetime_convert <- with_tz(
  datetime_convert, "Japan"
)
datetime_convert
...
marketwatch_latest_data <- subset(
  marketwatch_webpgs_datetimes, DiffHours < 1000
)
