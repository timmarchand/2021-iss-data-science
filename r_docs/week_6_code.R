...
### Clear memory
rm(list=ls(all=TRUE))
## install packages - un-comment as necessary
...
dat <- read_csv("https://tinyurl.com/iss-random") # our experimental data
...
jpn_ht <-  read_csv("https://tinyurl.com/iss-jpn-height-web") # for later comparisons


## let's save both into our project data folder
write_csv(x = dat,  file = "data/afica_guess.csv")
write_csv(x = jpn_ht,  file = "data/jpn_ht.csv")

## you can then load them again in future files using:
...
names <- c("Olesya","Tim","Xiaohua","Yuki")
names
...
# DIVERSIONS WITH OUR EXPERIMENTAL DATA ---------------------------------------------------
...
df_all <- dat
df_class <-dat %>% filter(dat$datasci)
...
class_height_summary <- data.frame(mean = mean(height), ## this returns the mean
...
ggplot(df_class) +
  ...
  theme_minimal()
## But is 11 enough to make that judgment?
...
## Much more overlap - with <50 data points, unlikely to be significant


## DIY 2
## Use the copy and paste and tweak method to create the same graphs for
## birthday vs guess in the df_class and df_all data sets
...
compare_heights
## No surprise the larger sample from the survey
## did a better job of matching the population mean
